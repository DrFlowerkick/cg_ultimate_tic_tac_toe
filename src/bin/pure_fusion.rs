...
fn main() {
    ...
    let time_out_codingame_input = Duration::from_millis(60_000);
            ...
            Ok((opponent_row, opponent_col)) => {
                ...
                if first_turn {
                    ...
                    if opponent_row >= 0 {
                        ...
                        mcts_ult_ttt
                            .mcts_config
                            .optimized_v05_set_exploration_boost(TicTacToeStatus::Second);
                    } else {
                        eprintln!("I'm first player.");
                        mcts_ult_ttt
                            .mcts_config
                            .optimized_v05_set_exploration_boost(TicTacToeStatus::First);
                    }
                } else {
...
struct UltTTTMCTSConfig {
    base_config: BaseConfig<TicTacToeStatus>,
}
impl UltTTTMCTSConfig {
    fn optimized_v05() -> Self {
        UltTTTMCTSConfig {
            base_config: BaseConfig {
                exploration_constant: 1.460,
                exploration_boost: [
                    (TicTacToeStatus::First, 1.219),
                    (TicTacToeStatus::Second, 0.866),
                ]
                .into(),
                progressive_widening_constant: 1.496,
                progressive_widening_exponent: 0.333,
                early_cut_off_depth: 14,
            },
        }
    }
    fn optimized_v05_set_exploration_boost(&mut self, my_playing_position: TicTacToeStatus) {
...
#[derive(Debug, Clone, PartialEq)]
struct BaseConfig<Player: GamePlayer> {
    exploration_constant: f32,
    exploration_boost: HashMap<Player, f32>,
